...
from .dbutils import (load_attributes, load_discharge, load_forcing, normalize_features, reshape_data)

from . import dbutils as db


class CamelsSqlite(Dataset):
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        df, area = db.load_forcing(self.db_path, self.gage_id)
        q_df = load_discharge(self.db_path, self.gage_id, area)

        # Handle cases where streamflow data may not start at the same index as the forcing data.
        date_mask = (df.index >= q_df.index[0]) & (df.index <= q_df.index[-1])
        df = df.loc[date_mask]
        df['QObs'] = q_df
        ...
        # x = np.array([
        #     df['prcp'].values, df['srad'].values, df['lrad'].values, df['swe'].values, df['tmax'].values,
        #     df['tmin'].values, df['vp'].values, df['et'].values
        # ]).T
        x = np.array([
            df['prcp'].values, df['srad'].values, df['tmax'].values, df['tmin'].values, df['vp'].values
        ]).T
    ...
    def _load_attributes(self) -> torch.Tensor:
        df = load_attributes(self.db_path, [self.gage_id])
        ...
        attributes = df.loc[df.index == self.gage_id].values
